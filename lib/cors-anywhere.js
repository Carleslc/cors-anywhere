...
function withCORS(headers, request) {
  const allowCredentials = !!request.corsAnywhereRequestState.cookies;
  headers['access-control-allow-origin'] = '*';
  headers['access-control-allow-credentials'] = allowCredentials;
  if (allowCredentials) {
    const originWhitelist = request.corsAnywhereRequestState.originWhitelist;
    if (originWhitelist.length > 0) {
      headers['access-control-allow-origin'] = originWhitelist.join(',');
    }
  }
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    cookies: false,                 // Disable cookies by default
